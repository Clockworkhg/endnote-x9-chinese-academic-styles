...
双击即可打开，不需要解压运行环境、不需要 PowerShell，也不需要管理员权限。47套 ENS、测试矩阵、64条测试文献和多语种参考类型配置均已嵌入 EXE。
...
2. 真正启动发布后的 EXE；
3. 通过`--self-test`验证47个嵌入样式及SHA-256；
4. 在临时目录执行安装、覆盖备份、状态检测、卸载恢复和配套资料导出；
5. 仅在全部检查通过后上传构建产物。

手动启动发版任务时，同一套 Windows 启动测试会再次运行；任务只有在全部测试通过后才创建相应标签和 Release。

## 当前格式库

| 类别 | 格式 |
|---|---|
| 综合/出版社 | 中国社会科学（2026）、综合性期刊文献引证技术规范、人民出版社、社会科学 |
| 中国传媒大学 | 中国传媒大学（人文社科脚注参考版，实验版） |
| 高校 | 清华大学（人文社科）、华东理工大学社管学院、南京农业大学人文社科、湖南大学（脚注）等 |
| 政治学与国际关系 | 政治学研究、世界经济与政治、外交评论、国际政治研究、国际安全研究、当代亚太等 |
| 历史、考古与文学 | 历史研究、世界历史、南方民族考古、文学评论、文艺争鸣、外国文学评论等 |
| 新闻传播 | 新闻与传播研究 |
| 法学 | 法学引注手册系列、中国政法大学、西南政法大学、中外法学、国际法研究 |
| 国家标准 | GB/T 7714—2015/2025注释版及无URL/DOI版 |
| 繁体中文 | 臺大中文學報 |

《中国社会科学（2026）》以 Alpha 2.7 为稳定基线，已完成 EndNote X9＋WPS 基础实测。工具箱覆盖固定上游版本中全部46套`class="note"`样式，并另含1套中传实验配置；其余样式已经过 ENS 结构与关键字段自动校验，属于预览/实验版。脚注序号、字体、字号和行距仍由 Word/WPS 控制。
...
app/       47套ENS、清单、测试资料及旧版0.2启动脚本
assets/    构建所需的基础样式、上游脚注目录快照、参考类型和来源示例
